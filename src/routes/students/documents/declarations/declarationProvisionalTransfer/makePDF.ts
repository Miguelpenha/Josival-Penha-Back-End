...
function makePDF(pdf: PDFKit.PDFDocument, student: IStudent, query: IDeclarationProvisionalTransferQuery) {
    const date = new Date().toLocaleDateString('pt-br', {
        dateStyle: 'long'
    })
    const { nextClass, progression } = query
    ...
    .moveDown(2)
    .font('Helvetica-Bold')
    .text('DECLARAÇÃO PROVISÓRIA DE TRANSFERÊNCIA', {
        align: 'center'
    })
    .font('Helvetica')
    .fontSize(13)
    ...
    .font('Helvetica')
    .text(`, matriculado(a) neste estabelecimento de Ensino no ${student.class.grade} do Ensino Fundamental. Tem direito a matrícula no `, {
        align: 'left',
        continued: true
    })
    .font('Helvetica-Bold')
...
`)
    ...
    .font('Helvetica-Bold')
    .text(student.class.grade, {
        align: 'left',
        continued: true
    })
    .font('Helvetica')
    .text(' nos termos da Lei ', {
        align: 'left',
        continued: true
    })
    .font('Helvetica-Bold')
    ...
    .font('Helvetica-Bold')
    .text((progression && progression != 'false') ? 'X' : ' ', {
        align: 'left',
        continued: true
    })
    .font('Helvetica')
    .text(' ) Progressão Plena ( ', {
        align: 'left',
        continued: true
    })
    .font('Helvetica-Bold')
